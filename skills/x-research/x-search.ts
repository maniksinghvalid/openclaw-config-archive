...
async function cmdSearch() {
  ...
  // Track raw count for cost (API charges per tweet read, before post-hoc filters)
  ...
  if (sortOpt !== "recent") {
    tweets = api.sortBy(tweets, sortOpt as any);
  }
  ...
  if (asJson) {
    ...
    const md = fmt.formatResearchMarkdown(query, tweets, { queries: [query] });
    console.log(md);
  } else {
