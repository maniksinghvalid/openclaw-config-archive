...
async function apiGet(url: string, retries: number = 3): Promise<RawResponse> {
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    if (res.status === 429) {
      if (attempt < retries) {
        console.error(`Rate limited (attempt ${attempt}/${retries}). Waiting 60s...`);
        await sleep(60_000);
        continue;
      }
      ...
      const waitSec = reset
        ...
        : 60;
      throw new Error(`Rate limited after ${retries} retries. Resets in ${waitSec}s`);
    }
