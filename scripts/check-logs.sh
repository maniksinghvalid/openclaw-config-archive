...
# Configuration
LOG_FILE=$(ls -t /home/claw/.openclaw/agents/main/sessions/*.jsonl | head -n 1)
...
echo "## 🚨 Error Summary (Last 1000 lines)" >> "$SUMMARY_FILE"
tail -n 1000 "$LOG_FILE" | grep -E "isError\":true|status\":\"error\"" | sed 's/.*"toolName":"\([^"]*\)".*/\1/' | sort | uniq -c >> "$SUMMARY_FILE"

echo "" >> "$SUMMARY_FILE"
echo "## 🔍 Detected Issues & Patterns" >> "$SUMMARY_FILE"

# Check for Brave Search 422
if tail -n 1000 "$LOG_FILE" | grep -E "isError\":true|status\":\"error\"" | grep -q "Brave Search API error (422)"; then
...
if tail -n 1000 "$LOG_FILE" | grep -E "isError\":true|status\":\"error\"" | grep -q "spotify-podcast-digest"; then
    if tail -n 1000 "$LOG_FILE" | grep -E "isError\":true|status\":\"error\"" | grep -q "No such file or directory"; then
...
if tail -n 1000 "$LOG_FILE" | grep -E "isError\":true|status\":\"error\"" | grep -q "bun: No such file or directory"; then
